     ...
     H*     36: Hide EOS field in SFL if NULL.
     H*     37: Color EOS field red in SFL if EOS.
     ...
     H*     46: Hide EOS field in DSREC if NULL. (DSPF)
     H*     47: Color EOS field red in DSREC if EOS. (DSPF)
     H* 50..54: SFL Sorting.
     H* 60..69: Detail record cursor placement. (DSPF)
     H*- Other Stuff:
     H*     71: READC from Subfile EOF.
     H*     72: READE for replacing versions EOF.
     ...
     F*
     F* Nicely sorted LF for the SFL.
     FOSMATCH1LFIF   E           K DISK
     FOSMATCH2LFIF   E           K DISK
     FOSMATCH3LFIF   E           K DISK
     FOSMATCH4LFIF   E           K DISK
     FOSMATCH5LFIF   E           K DISK
     F*
     F* For quick find of maximum ID.
     FOSMATCHMAXIF   E           K DISK    USROPN
     F*
     F* For replacing versions.
     FOSMATCHRPLIF   E           K DISK    USROPN
     F*
     F* Display file with multiple subfiles among other record formats.
     FOSMATCHDF CF   E             WORKSTN
     ...
     C     *LOVAL        SETLL     SFLTBL1
     C     SORTKEY       WHENEQ    1
     C     *LOVAL        SETGT     SFLTBL2
     C     SORTKEY       WHENEQ    2
     C     *LOVAL        SETGT     SFLTBL3
     C     SORTKEY       WHENEQ    3
     C     *LOVAL        SETGT     SFLTBL4
     C     SORTKEY       WHENEQ    4
     C     *LOVAL        SETGT     SFLTBL5
     C                   ENDSL
     C*
     C                   ENDSR
     C*************************************************************************
     ...
     C*
     C* Shorten Version.
     C* Now, do the calc and magic.
     C                   EVAL      STRLEN_CUR=%LEN(%TRIMR(VERSION))
     C     STRLEN_CUR    IFGT      STRLEN_MAX
     C     STRLEN_MIN    SUBST     VERSION:1     VERSION$
     C                   CAT       '...':0       VERSION$
     C                   ELSE
     C     STRLEN_MAX    SUBST     VERSION:1     VERSION$
     C                   ENDIF
     C*
     C* Check for NULL fields and overwrite with *BLANK so fields appear empty.
     C                   EXSR      CHECKNULL
     C*
     C* Display EOS field if not NULL.
     C                   IF        %NULLIND(EOS)
     C                   MOVE      *ON           *IN36
     C                   ELSE
     C                   MOVE      *OFF          *IN36
     C* Display EOS in color if EOS.
     C                   MOVE      EOS           EOSDAT
     C     CURDAT        SUBDUR    EOSDAT        STAMPDIFF:*D
     C     STAMPDIFF     IFGE      *ZERO
     C                   MOVE      *ON           *IN37
     C                   ELSE
     C                   MOVE      *OFF          *IN37
     C                   ENDIF
     ...
     C                   MOVE      *ON           *IN(DSPFUL)
     C*
     C* First, calculate needed offsets/cur positions.
     C                   EVAL      STRLEN_MAX=%LEN(VERSION$)
     C     STRLEN_MAX    SUB       3             STRLEN_MIN
     C                   MOVE      *BLANK        VERSION$
     C*
     ...
     C     *IN98         OREQ      *ON
     C*
     C* Prepare empty fields to be NULL. Note: Complex handler for date field
     C*  logic included there!
     C                   EXSR      SETNULL
     C*
     ...
     C     *IN78         IFEQ      *ON
     C     ID            CHAIN(EN) OSMATCHTBL
     C                   ELSE
     C     ID            CHAIN(E)  OSMATCHTBL
     C                   ENDIF
     C*
     C                   EVAL      FSTAT=%STATUS(OSMATCHPF)
     C     FSTAT         IFGT      *ZERO
     ...
     C                   SELECT
     C     CSFLD         WHENEQ    'VERSION'
     C                   MOVE      *ON           *IN60
     C     CSFLD         WHENEQ    'MODEL'
     C                   MOVE      *ON           *IN61
     C     CSFLD         WHENEQ    'RAM'
     C                   MOVE      *ON           *IN62
     C     CSFLD         WHENEQ    'FLASH'
     C                   MOVE      *ON           *IN63
     C     CSFLD         WHENEQ    'EOS$'
     C                   MOVE      *ON           *IN64
     C                   ENDSL
     ...
     C*  compiler flags!
     C*
     C* Special handling of EOS PF field vs. EOS$ DSPF field.
     C                   IF        %NULLIND(EOS)
     C                   MOVEL     *BLANK        EOS$
     C                   ELSE
     C                   MOVE      EOS           EOS$
     ...
     C*  compiler flags!
     C*
     C     EOS$          IFEQ      '0001-01-01'
     C                   MOVEL     *BLANK        EOS$
     C                   ENDIF
     C*
     C     EOS$          IFEQ      *BLANK
     C                   MOVE      '0001-01-01'  EOS
     C                   EVAL      %NULLIND(EOS) = *ON
     C                   ELSE
     C                   MOVE      EOS$          EOS
     C                   EVAL      %NULLIND(EOS) = *OFF
     C                   ENDIF
     C*
     C                   ENDSR
     C*************************************************************************
     C     REPLACEVERS   BEGSR
     C* Replace one version string with another.
     C                   OPEN      OSMATCHRPL
     C*
     C* Load data of the current selected record from the database file. Maybe
     C*  condition the data and WRITE it into the DSPF.
     C*
     C     ID            CHAIN(EN) OSMATCHTBL
     ...
     C                   SELECT
     C     FSTAT         WHENEQ    12
     C* Record could not be found (anymore?).
     C                   MOVE      *ON           *IN91
     C                   ENDSL
     C                   MOVE      *ON           *IN99
     C                   LEAVESR
     C                   ENDIF
     C*
     C                   MOVE      VERSION       VERS#OLD
     C                   MOVE      VERSION       VERS#NEW
     C*
     C                   EXFMT     REPVERFRM
     C*
     C* Whee! User pressed a key!
     ...
     C                   ENDIF
     C*
     C* Do nothing if the form wasn't changed.
     C     *IN28         IFEQ      *OFF
     C                   MOVE      *ON           *IN95
     C                   MOVE      *ON           *IN99
     C                   CLOSE     OSMATCHRPL
     ...
     C                   ENDIF
     C*
     C* Psoition outselves for further READEs.
     C     VERS#OLD      SETLL     REPLACETBL
     C*
     C* Get current timestamp.
     C                   TIME                    STAMP
     C                   MOVEL     STAMP         CHANGED
     C*
     C* Loop through the read-by-version-file until EOF for given VERSION.
     C     *ZERO         DOWEQ     *ZERO
     C     VERS#OLD      READE     REPLACETBL                             72
     C     *IN72         IFEQ      *ON
     C                   LEAVE
     C                   ENDIF
     C*
     C* Read-and-lock our update file.
     C     ID            CHAIN(E)  OSMATCHTBL
     C*
     C* Handle File errors.
     C                   EVAL      FSTAT=%STATUS(OSMATCHPF)
     C     FSTAT         IFGT      *ZERO
     C* FIXME: How to cope with errors in here?
     C                   ITER
     C                   ENDIF
     C*
     C                   MOVE      VERS#NEW      VERSION
     C*
     C                   UPDATE(E) OSMATCHTBL
     C                   EVAL      FSTAT=%STATUS(OSMATCHPF)
     C     FSTAT         IFGT      *ZERO
     ...
     C                   ENDDO
     C*
     C* Finish by closing our read-by-version-file.
     C                   CLOSE     OSMATCHRPL
     C*
     C                   ENDSR
     C*************************************************************************
